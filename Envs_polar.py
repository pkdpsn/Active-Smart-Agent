...
'''
This is a discretised(in development) grid world environment where the agent is placed in a grid world with a visibility range of d. 
...
class rlEnvs(Env):
    ...
    def _return_state(self,x,y):
        state_radius = np.sqrt(x**2 + y**2)
        state_theta = np.arctan2(y,x)
        if state_theta<0:
            state_theta = 2*np.pi + state_theta
        closest_radius = self.radius[np.abs(self.radius-state_radius).argmin()]
        closest_theta = self.theta_range[np.abs(self.theta_range-state_theta).argmin()] 
        return closest_radius, closest_theta
    ...
    def step(self, action: int):
        ...
        elif self.velocity<=0 or self.total_time>=10  :
            self.reward = -500
            self.truncated = True
        # elif True:
